...
/* ---------- стан ---------- */
const S = {
  mode: 'loading', role: null, uid: null, me: null, err: null,
...
function render(top) {
  ...
  else if (S.mode === 'error') html = box(`<h1>Не вдалося відкрити</h1><p class="muted">Перевір інтернет і спробуй ще раз.</p><button class="btn" style="margin-top:14px" data-act="retry">Спробувати ще</button>`);
...
async function realAuth() {
  const r = await fetch(CFG.SUPABASE_URL + '/functions/v1/telegram-auth', { method: 'POST', headers: { 'Content-Type': 'application/json', apikey: CFG.SUPABASE_ANON_KEY }, body: JSON.stringify({ initData: tg.initData }) });
  const j = await r.json().catch(() => ({}));
  if (!r.ok || !j.token_hash) throw new Error(j.error || 'auth_failed');
...
async function boot() {
  ...
  try {
    if (DEMO) {
      ...
      if (!CFG.SUPABASE_URL || !CFG.SUPABASE_ANON_KEY || CFG.SUPABASE_URL.indexOf('ТВІЙ') !== -1) throw new Error('config');
